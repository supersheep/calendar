...
// 可选日期
var CLASS_AVAIL = [PREFIX, "availiable"].join(CLASS_SPLITER);
// 星期
...
var Calendar = Class({
	...
	initialize: function(container, options) {
		...
		this.set("defaultDate", options.date || today);
		this.set("renderDay", options.renderDay);
		this.set("availiableDays", options.availiableDays);
		self._render();
		self._bind();
	},
	...
	_refreshDay: function(td) {
		...
		if (this._isLastMonth(date)) {
			...
			if (this.get('availiableDays')(date)) {
	...
	_renderDay: function(date) {
		...
		self._refreshDay(td);
		self.customRenderDay && self.customRenderDay(td, date);
		return td;
	},
	_render: function() {
